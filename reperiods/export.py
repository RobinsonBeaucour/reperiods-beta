...
def save_RP(folder_path, temporal_data, sep=','):
    ...
    weights = []
    for i, RP in enumerate(temporal_data.RP):
    ...
    weights_df = pd.DataFrame(weights, index=[f'RP_{str(i).zfill(2)}' for i in range(len(temporal_data.RP))], columns=['value'])
